...
public class SseClient {
    ...
    public SseEmitter createSse(String uid) {
        ...
        log.info(String.valueOf(sseEmitter.getTimeout()));
        //完成后回调
        ...
        try {
            sseEmitter.send(SseEmitter.event().reconnectTime(60*60*1000L));
        } catch (IOException e) {
            e.printStackTrace();
        }
        sseEmitterMap.put(uid, sseEmitter);
        log.info("[{}]创建sse连接成功！"+uid+sseEmitterMap.keySet().toString());
        return sseEmitter;
    }
    ...
    public void sendMessage(String uid, String messageId, String message) {
        ...
        try {
            sseEmitter.send(SseEmitter.event().id(messageId).reconnectTime(3000L).data(message));
